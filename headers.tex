%%%%%%%%%%%%%%% INDEX %%%%%%%%%%%%%%%

\usepackage{imakeidx}

%%%%% Code magique pour faire marcher dotted_mode.ist
%% D'après https://tex.stackexchange.com/questions/594568/how-to-integrate-a-tabular-environment-in-the-index
\setlength{\columnseprule}{0.4pt}
\newsavebox\ltmcbox
\newlength\mysavecolroom
%% Utilisé dans les fichiers .ist afin d'ignorer automatiquement 
%% les instances de \\ dans les index qui emploient longtable, ce qui casserait la table.
\newcommand{\ignoreNL}[1]{} 
%% Macro à employer pour effectuer des sauts de lignes dans les noms indexés si ce nom est indexé avec longtable (p.ex. dotted_mode.ist)
\newcommand{\idxnewline}{\newline\null\hspace{4mm}}
%%%%% Fin du code magique

%% Définition des index 
\indexsetup{
	level=\section*,
	toclevel=section,
	noclearpage,
	othercode=\footnotesize\thispagestyle{empty}
}
\makeindex[name=A,title=Index Antiphonarum, columns=2,columnseprule, options=-s dotted_mode.ist]
\makeindex[name=H,title=Index Hymnorum, columns=2,columnseprule, options=-s dotted_mode.ist]
\makeindex[name=R,title=Index Responsoriorum, columns=2,columnseprule, options=-s dotted_mode.ist]
\makeindex[name=F,title=Index Festorum, columns=2,columnseprule, options=-s dotted.ist]

%% wrapper pour printindex qui rectifie les en-têtes et le style de page
\newcommand{\cprintindex}[2]{
	\setheader{#2}
	\pagestyle{fancy}
	\thispagestyle{empty}
	\printindex[#1]
}

%%%%%%%%%%%%%%% GÉOMÉTRIE %%%%%%%%%%%%%%%

%KDP 6inx9in sans fonds perdus
\usepackage[paperwidth=6in, paperheight=9in]{geometry}
%A5
%\usepackage[a5paper]{geometry}

% Marges pour le format 6in x 9in
\geometry{
inner=25mm,
outer=14mm,
top=14mm,
bottom=14mm,
headsep=3mm,
}

% Marges pour le format A5
% \geometry{
% inner=15mm,
% outer=13mm,
% top=13mm,
% bottom=13mm,
% headsep=3mm,
% }

%%%%%%%%%%%%%%% PACKAGES STANDARD %%%%%%%%%%%%%%%

\usepackage{fontspec}
\usepackage[nolocalmarks]{polyglossia}
%% utiliser la typographie francaise est une bonne idée même pour le latin (l'option latin existe mais marche moins bien)
\setdefaultlanguage[variant=french, frenchitemlabels=false]{french}
\usepackage[table]{xcolor}
\usepackage{fancyhdr}
\usepackage{titlesec}
\usepackage{setspace}
\usepackage{expl3}
\usepackage{hyperref}
\usepackage{refcount}
\usepackage{needspace}
\usepackage{etoolbox}
\usepackage{enumitem}
\usepackage{lettrine}
\usepackage{longtable}
\usepackage{luacode}
\usepackage{paracol}
\usepackage{alltt}
\usepackage{parskip}

%%%%%%%%%%%%%%% POLICES DE CARACTÈRES %%%%%%%%%%%%%%%
\setmainfont[Ligatures=TeX]{Plantin MT Pro} % changer pour changer la police
\setstretch{1.05} % changer pour augmenter ou diminuer l'interligne

%%%%%% Personnalisation du crénage de Plantin MT Pro

\begin{luacode*}
local glyph = node.id("glyph")
local glue  = node.id("glue")
local kern  = node.id("kern")

local function fix_space_T(head)
  local n = head
  while n do
    if n.id == glue then
      local next = n.next
      if next and next.id == glyph then
        local char = unicode.utf8.char(next.char)
        if char == "T" or char == "V" then
          -- Création du kern
          local k = node.new(kern)
          k.kern = 1.5 * 65536  -- 1pt

          -- Insertion du kern entre l'espace et le T
          k.next = next
          n.next = k
        end
      end
    end
    n = n.next
  end
  return head
end

luatexbase.add_to_callback("pre_linebreak_filter", fix_space_T, "fix_space_T")
luatexbase.add_to_callback("hpack_filter", fix_space_T, "fix_space_T_hpack")
\end{luacode*}

%%%%%%%%%%%%%%% CONFIGURATION GREGORIO %%%%%%%%%%%%%%%

\usepackage[forcecompile]{gregoriotex}

%% On emploie la règle dominicaine sur les altérations valables jusqu'à la fin de la ligne
\gresetalterationeffect{line}

%% On autorise les orphelins et les veuves, mais on décourage les veuves.
\grechangecount{widowpenalty}{20}
\grechangecount{clubpenalty}{-20}

%% Variable globale contenant le nom de fichier de la pièce en train d'être imprimée, qui est aussi son label pour référence interne
\global\def\labelbuffer{}
%% Variable globale contenant le nom de l'index (A, H, R) pertinent pour la pièce en train d'être imprimée, s'il y en a un
\global\def\indexnamebuffer{}
%% Variable globale contenant le nom de la pièce (tel qu'indiqué dans l'en-tête name:) en train d'être imprimée
\global\def\namebuffer{}
%% Variable globale contenant le mode (+ differentia si antienne) de la pièce en train d'être imprimée, pour l'imprimer en annotation
\global\def\annotationmode{}
%% Variable globale contenant l'annotation à imprimer à la place du mode, si celle-ci est fournie
\global\def\annotationmodeoverride{}
%% Variable globale contenant le mode (+ parfois differentia si un mode a plusieurs pointages) pour déterminer le prochain psaume pointé.
\global\def\nextpsalmmode{}
%% Variable globale contenant le mode de la pièce en train d'être imprimée, pour l'ajouter à son entrée d'index.
\global\def\indexedmode{}

%% Cette commande remet à zéro le style des initiales après une modification de style (lettrine)
\newcommand{\resetinitialstyle}{%
	\gresetinitiallines{1}%
	\grechangestyle{initial}{\begingroup\fontsize{40}{40}\selectfont}[\endgroup]%
	\grechangedim{annotationraise}{\defaultannotationraise}{fixed}%
}
%% Appel des polices de lettrines avec les différents stylisticsets de SolesmesCaps
\newfontfamily\solesmescaps[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
]{SolesmesCaps}
\newfontfamily\descleeflora[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
]{DescleeFlora}
\newfontfamily\solesmescapsround[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
  StylisticSet = {1} % ss01 = table ROUNDER-1
]{SolesmesCaps}
\newfontfamily\solesmescapsalt[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
  StylisticSet = {2} % ss02 = table VARIANTS-1
]{SolesmesCaps}
\newfontfamily\solesmescapsroundalt[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
  StylisticSet = {1, 3} % ss03 = table ROUNDERVARIANTS-1
]{SolesmesCaps}
\newfontfamily\solesmescapsspecial[
  Path = ./,
  Extension = .ttf,
  Ligatures = TeX,
  StylisticSet = {4} % ss04 = table SPECIFIC-1
]{SolesmesCaps}
%% Cette commande permet de modifier le niveau d'initiale de la prochaine pièce.
\newcommand{\cantusilluminatio}[2][normal]{
%% #1 (optionnel) : variante demandée pour la police d'initiales: "round", "alt", "roundalt", "special"
%% #2: niveau d'initiale attendue. 1 => initiale ornée de hauteur 1 ligne. 2 => hauteur 2 lignes.
	\ifstrequal{#2}{1}{%
		\gresetinitiallines{1}%
		\grechangestyle{initial}{\begingroup\descleeflora\fontsize{53}{53}\selectfont}[\endgroup]%
		\grechangedim{annotationraise}{4.3mm}{fixed}%
	}{}%
	\ifstrequal{#2}{2}{%
		\gresetinitiallines{2}%
		\ifstrequal{#1}{normal}{%
			\grechangestyle{initial}{\begingroup\solesmescaps\fontsize{125}{125}\selectfont}[\endgroup]%
		}{}%
		\ifstrequal{#1}{round}{%
			\grechangestyle{initial}{\begingroup\solesmescapsround\fontsize{125}{125}\selectfont}[\endgroup]%
		}{}%
		\ifstrequal{#1}{alt}{%
			\grechangestyle{initial}{\begingroup\solesmescapsalt\fontsize{125}{125}\selectfont}[\endgroup]%
		}{}%
		\ifstrequal{#1}{roundalt}{%
			\grechangestyle{initial}{\begingroup\solesmescapsroundalt\fontsize{125}{125}\selectfont}[\endgroup]%
		}{}%
		\ifstrequal{#1}{special}{%
			\grechangestyle{initial}{\begingroup\solesmescapsspecial\fontsize{125}{125}\selectfont}[\endgroup]%
		}{}%
		\grechangedim{annotationraise}{4mm}{fixed}%
	}{}
}


%% Cette section prend un header office-part (Antiphona, etc.) détermine son index
\newcommand{\abbrev}[3]{%
  \ifstrequal{#1}{#2}{%
	\global\def\indexnamebuffer{#3}%
  }%
  {}%
}
\newcommand{\officepartannotation}[1]{%
  \global\def\indexnamebuffer{NoIndex}%
  \abbrev{#1}{Antiphona}{A}%
  \abbrev{#1}{Hymnus}{H}%
  \abbrev{#1}{Responsorium breve}{R}%
  \abbrev{#1}{Responsorium}{R}%
}

%% Cette section prend un header mode (1a...), annote la pièce avec ce mode, 
%% et en prend note dans \nextpsalmmode pour aller chercher le bon psaume après.
%% étant la dernière section exécutée immédiatement avant l'impression de la pièce (car dernier header dans l'ordre),
%% elle porte également l'indexation avec le nom, le mode et l'index identifiés.
\newcommand{\changemode}[5]{%
  \ifstrequal{#1}{#2}{%
	\global\def\nextpsalmmode{#3}%
	\global\def\annotationmode{#4}%
	\global\def\indexedmode{#5}%
  }%
  {}%
}
\newcommand{\modeannotation}[1]{
  % 1e colonne: champ mode du GABC
  % 2e colonne: mode de pointage du psaume
  % 3e colonne: mode imprimé au-dessus de l'initiale
  % 4e colonne: mode imprimé dans l'entrée d'index
  \global\def\annotationmode{#1}
  \global\def\nextpsalmmode{#1}
  \changemode{#1}{1a}{1}{I a}{1}
  \changemode{#1}{1b}{1}{I b}{1}
  \changemode{#1}{1c}{1}{I c}{1}
  \changemode{#1}{1}{1}{I}{1}
  \changemode{#1}{2}{2}{II}{2}
  \changemode{#1}{3a}{3a}{III a}{3}
  \changemode{#1}{3b}{3b}{III b}{3}
  \changemode{#1}{3}{3}{III}{3}
  \changemode{#1}{4a}{4}{IV a}{4}
  \changemode{#1}{4b}{4}{IV b}{4}
  \changemode{#1}{4}{4}{IV}{4}
  \changemode{#1}{5}{5}{V}{5}
  \changemode{#1}{6}{6}{VI}{6}
  \changemode{#1}{7a}{7}{VII a}{7}
  \changemode{#1}{7b}{7}{VII b}{7}
  \changemode{#1}{7}{7}{VII}{7}
  \changemode{#1}{8a}{8a}{VIII a}{8}
  \changemode{#1}{8b}{8b}{VIII b}{8}
  \changemode{#1}{8}{8}{VIII}{8}
  \changemode{#1}{Per.}{per}{Per.}{P}
  % si l'annotation de mode a été explicitement fournie (argument #1 de \cantus non vide),
  % on l'imprime, sinon, on imprime l'annotation de mode déterminée automatiquement.
  \expandafter\ifstrequal\expandafter{\annotationmodeoverride}{no-override}%
	{\greannotation{\annotationmode}}%
	{\greannotation{\annotationmodeoverride}}%
  %% indexation
  \ifstrequal{H}\expandafter{\indexnamebuffer}{%
    \index[\indexnamebuffer]{{\namebuffer}@\indexedmode & \namebuffer}%
  }{}%
  \ifstrequal{A}\expandafter{\indexnamebuffer}{%
    \index[\indexnamebuffer]{{\namebuffer}@\indexedmode & \namebuffer}%
  }{}%
  \ifstrequal{R}\expandafter{\indexnamebuffer}{%
    \index[\indexnamebuffer]{{\namebuffer}@\indexedmode & \namebuffer}%
  }{}%
}

%% Cette section prend un header name (le nom de la pièce),
%% assigne son label (le nom de fichier) à la pièce, et un nom au label (le nom de la pièce),
%% et mémorise le nom dans \namebuffer pour indexation future une fois qu'on a le mode.
\begin{luacode*}
function delete_newline ( s )
   s = string.gsub ( s, 'newline', '')
   s = string.gsub ( s, 'hspace', '')
   s = string.gsub ( s, '4mm', '')
   s = string.gsub ( s, 'protect', '')
   s = string.gsub ( s, 'hbox', '')
   s = string.gsub ( s, [[\]], '')
   s = string.gsub ( s, "{}", '')
   tex.sprint ( s )
end
\end{luacode*}
\makeatletter
\newcommand{\namecapture}[1]{%
  %% this prevents page breaks between the phantom section and its label, and the actual score.
  \needspace{4\baselineskip}%
  \protected@edef\@currentlabelname{\directlua{delete_newline(\luastring{#1})}}%
  \phantomsection%
  \label{\labelbuffer}%
  \global\def\namebuffer{#1}
}
\makeatother

\gresetheadercapture{office-part}{officepartannotation}{}
\gresetheadercapture{mode}{modeannotation}{string}
\gresetheadercapture{name}{namecapture}{}

\newcommand{\cantusparvus}[1]{
  \gresetinitiallines{0}
  \setmainfont{Plantin MT Pro}[Scale=1] % changer cette valeur à 1.15 ou 1.20 pour augmenter la taille du texte des chants de 15% ou 20%
  \gregorioscore{gabc/#1}
  \setmainfont{Plantin MT Pro}[Scale=1]
  \resetinitialstyle
}

\newcommand{\cantus}[3][no-override]{
  %% #1 (optionnel) : l'annotation à faire figurer à la place du mode
  %% #2 : le nom de fichier (sans .gabc) qui sera la référence de la pièce
  %% #3 : l'annotation à faire figurer au-dessus du mode
  \renewcommand{\labelbuffer}{#2}
  \renewcommand{\annotationmodeoverride}{#1}
  \greannotation{#3}
  \vspace{6pt plus 2pt minus 2pt}
  \setmainfont{Plantin MT Pro}[Scale=1] % changer cette valeur à 1.15 ou 1.20 pour augmenter la taille du texte des chants de 15% ou 20%
  \gregorioscore{gabc/#2}
  \setmainfont{Plantin MT Pro}[Scale=1]
  \resetinitialstyle
}

%%%%%% Géométrie des annotations

\grechangestyle{annotation}{\footnotesize}
\newcommand{\defaultannotationraise}{0mm}
\grechangedim{annotationraise}{\defaultannotationraise}{fixed}


%%%%%%%%%%%%%%% COLUMN MANAGEMENT %%%%%%%%%%%%%%%

%% Commande pour s'autoriser à avoir de l'espace horizontal entre
%% les mots au-delà des standards normalement acceptables
\sloppy

%% Environnement psalmodia qui encadre tous les psaumes imprimés sur deux colonnes.
%% Pour le moment, ne prend pas d'argument, et se contente de passer la mise en page sur deux colonnes.
\newenvironment{psalmodia}{\begin{multicols}{2}}{\end{multicols}}

%% Commande pour imprimer une liste itémisée (psaumes, cantiques)
\newcommand{\psalmverseindent}{5pt}
\newcommand{\itemizedlist}[2][]{
	%% #1 (optionnel) : titre à imprimer en en-tête
	%% #2 : fichier itémisé à imprimer sur deux colonnes
	{\centering
	#1\par
	}
	\begin{itemize}[
		label=\null, 
		leftmargin=0pt, 
		itemindent=\psalmverseindent, 
		labelsep=0pt, 
		labelwidth=0pt, 
		rightmargin=0pt, 
		parsep=0pt, 
		itemsep=0pt plus 1pt]
	\input{#2}%
	\end{itemize}%
}

%% commande appelée au premier \item de chaque fichier de psaume 
%% pour y insérer une lettrine de hauteur 2
%% #1: initiale 
%% #2: reste du premier mot
%% #3: reste du premier verset
\newcommand{\pslettrine}[3]{%
	\hspace{-\psalmverseindent}\parbox[b]{\linewidth}{\lettrine{#1}{}#2 #3}%
}

\newcommand{\psalmusinpsalmodia}[2][\nextpsalmmode]{
	%% #1 (optionnel): ton du psaume si différent de \nextpsalmmode
	%% #2: numéro du psaume
	\phantomsection
	\label{Psalmus#2_#1}
	%\index[P]{#2 (tonus #1)} %% seulement si on indexe les psaumes
	\itemizedlist[\textbf{Psalmus #2}]{psalmi/#2_#1}
}

\newcommand{\canticuminpsalmodia}[4][\nextpsalmmode]{
	%% #1 (optionnel): ton du cantique si différent de \nextpsalmmode
	%% #2: nom de fichier du cantique
	%% #3: titre du cantique tel qu'imprimé
	%% #4: référence biblique
	\phantomsection
	\label{Canticum#2_#1}
	%\index[C]{#2 (tonus #1)} %% seulement si on indexe les cantiques
	\itemizedlist[\textbf{#3}\\\rubric{#4}]{psalmi/#2_#1}
}

\newcommand{\psalmus}[2][\nextpsalmmode]{
	%% #1 (optionnel): ton du psaume si différent de \nextpsalmmode
	%% #2: numéro du psaume
	\begin{psalmodia}%
	\psalmusinpsalmodia[#1]{#2}
	\end{psalmodia}%
}

\newcommand{\canticum}[4][\nextpsalmmode]{
	%% #1 (optionnel): ton du cantique si différent de \nextpsalmmode
	%% #2: nom de fichier du cantique
	%% #3: titre du cantique tel qu'imprimé
	%% #4: référence biblique
	\begin{psalmodia}%
	\canticuminpsalmodia[#1]{#2}{#3}{#4}
	\end{psalmodia}%
}



%%%%%%%%%%%%%%% EN-TÊTES DE PAGES %%%%%%%%%%%%%%%

\pagestyle{fancy}
\fancyhead{}
\fancyfoot{}
\renewcommand{\headrulewidth}{1pt}
\setlength{\headheight}{20pt}
\newcommand{\themark}{}
\fancyhead[RO, LE]{\thepage}
\fancyhead[CO, CE]{\themark}

% cette commande est appelée pour définir l'en-tête de page (p.ex. en appelant \festum)
% la surcharge de \hyphenpenalty est nécessaire sur certaines anciennes versions de gregorio
% voir https://tex.stackexchange.com/questions/581013/lualatex-hyphenation-issue-in-fancyhdr-with-gregoriotex-and-multicols-latin-te
\newcommand{\setheader}[1]{
	\renewcommand{\themark}{\hyphenpenalty=50{\sc#1}}
}

%%%%%%%%%%%%%%% STYLES %%%%%%%%%%%%%%%

%%%%%%%%%%%% Styles globaux

%% Pas d'indentation des paragraphes
\setlength{\parindent}{0mm}
%% Espacement limité entre les paragraphes
\setlength{\parskip}{6pt plus 3pt minus 1pt}

%% Macro to print the name of a score in normal characters inside a \rubric
\newcommand{\normaltext}[1]{{\normalfont\normalcolor #1}}
\newcommand{\scorename}[1]{\normaltext{\nameref{M-#1}}}

%% Macros to print preparatory and accented syllables in psalms
\newcommand{\sylac}[1]{{\bfseries{}#1}}
\newcommand{\sylprep}[1]{{\itshape{}#1}}

\newcommand{\sylchange}[1]{{\bfseries{}#1}}
\newcommand{\sylpass}[1]{{\itshape{}#1}}

\input{macros}

%%%%%%%%%%%%%% Styles des titres

\newfontfamily\plantinlighttitles[
   Scale	=	1.05,
   ItalicFont     = PlantinMTProLightIt,
   BoldFont       = PlantinMTProRg,
   BoldItalicFont = PlantinMTProRgIt]{PlantinMTProLight}
\newcommand{\festum}[5]{
	%% #1 : pré-titre (date du jour, pour les fêtes fixes)
	%% #2 : titre principal de la fête
	%% #3 : sous-titre (classe de la fête) 
	%% #4 : texte à afficher dans l'en-tête de page
	%% #5 : taille de titre (0 pour le plus gros à 3 pour le plus petit)
	\begin{center}
	\ifstrequal{#5}{0}{
		\needspace{9\baselineskip}%
		\ifstrequal{#1}{}{}{\large{\scshape \textcolor{gregoriocolor}{#1}}\par}%
		{\huge \textsc{#2}}\par%
		\ifstrequal{#3}{}{}{\large \textcolor{gregoriocolor}{#3}\par}%
		\thispagestyle{empty}
	}{}
	\ifstrequal{#5}{1}{
		\needspace{9\baselineskip}%
		\vspace{-0.45mm}%
		\ifstrequal{#1}{}{}{\normalsize{\scshape \textcolor{gregoriocolor}{#1}}\par}%
		{\Large \plantinlighttitles{\par\uppercase{#2}\par}}%
		\vspace{0.4mm}%
		\ifstrequal{#3}{}{}{\normalsize \textcolor{gregoriocolor}{#3}\par\nopagebreak}%
	}{}
	\ifstrequal{#5}{2}{
		\needspace{8\baselineskip}%
		\ifstrequal{#1}{}{}{\normalsize{\scshape \textcolor{gregoriocolor}{#1}}\par}%
		\vspace{0.45mm}%
		{\Large {#2}}\par\nopagebreak%
		\ifstrequal{#3}{}{}{\normalsize \textcolor{gregoriocolor}{#3}\par\nopagebreak}%
	}{}
	\ifstrequal{#5}{3}{
		\needspace{7\baselineskip}%
		\ifstrequal{#1}{}{}{{\noindent\makebox[0pt][l]{\large #1}}}%
		\makebox[\textwidth][c]{\vtop{\hsize=.9\textwidth \large #2}}\par\nopagebreak%
		\ifstrequal{#3}{}{}{\normalsize \textcolor{gregoriocolor}{#3}\par\nopagebreak}%
	}{}
	\end{center}
	%% définition de l'en-tête de page
	\phantomsection
	\setheader{#4}
	%% si une date est indiquée, on indexe
	\ifstrequal{#1}{}{}{\index[F]{#2}} %% pas beau: rajouter un argument pour le nom indexé de la fête, ou déduire ce nom en coupant S. devant
}

%%%%%%%%%%%%% Styles des paragraphes spéciaux (capitulum oratio etc.)

%% Magie profonde pour pouvoir insérer \drop devant un paragraphe, et que cela insère une lettrine.
\def\drop #1#2 {% note the space before {
  \lettrine{#1}{#2} % a trailing space
}
\def\rdrop #1#2 {% note the space before {
  \lettrine{\textcolor{gregoriocolor}{#1}}{#2} % a trailing space
}

%%%%%%%%%%%%% Style de la TOC

\usepackage{titletoc}

\titlecontents{chapter}
  [-0.5em]                       % indentation
  {\normalfont}                 % style du texte dans la TOC (pas de gras)
  {\contentslabel{1.5em}}       % numéro + espace
  {}                            % avant le texte
  {\hfill\contentspage}         % numéro de page aligné à droite

%%%%%%%%%%%%%%% SUBFILES %%%%%%%%%%%%%%%

\usepackage{xr}
\usepackage{subfiles}

%% When we start a new subfile (new chapter), 
%% we start on a new page (with blank filling on the previous page) and create a corresponding label.
\newcommand{\customsubfile}[1]{\newpage\label{#1}\thispagestyle{empty}\subfile{#1}}